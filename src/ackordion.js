...
(function(root, factory) {
  ...
  function getAckordionTabPanel(element) {
    ...
    while (root) {
      ...
      if (root && root.getAttribute('role') === 'tabpanel') return root;
    }
    return undefined;
  }
  ...
  var Accordion = function(config) {
    ...
    if (!self.root) {
      error(
        'ackordion error - component not found in html with id: ' + self.id
      );
      return;
    }
    ...
    if (typeof config === 'object') {
      ...
      if (config.autoClosePrevious === false) self.autoClosePrevious = false;
    }
    ...
    if (id) {
      ...
      var openedTabs = qsa('.ackordion--active', self.root);
      if (openedTabs && openedTabs.length) {
        self.previous = openedTabs[0];
      }
  ...
  function toggle(element) {
    ...
    if (
      accordion &&
      accordion.autoClosePrevious &&
      accordion.previous &&
      tabpanel !== accordion.previous
    ) {
    ...
    // update previous
    if (accordion) accordions[id].previous = tabpanel;
  }
